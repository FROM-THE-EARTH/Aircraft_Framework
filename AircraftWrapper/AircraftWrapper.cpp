...
template<class T> bool AircraftWrapper<T>::initialize()
{
  // IM920
...
template<class T> void AircraftWrapper<T>::update()
{
...
template<class T> bool AircraftWrapper<T>::isReady(bool showDetail)
{
...
template<class T> void AircraftWrapper<T>::end()
{
  // end processing
}

template<class T> void AircraftWrapper<T>::waiting()
{
  beginRecord();
}

template<class T> void AircraftWrapper<T>::waitingLaunch()
{
  if (launchCondition())
  {
...
template<class T> void AircraftWrapper<T>::inFlight()
{
  if (detachCondition())
  {
...
template<class T> void AircraftWrapper<T>::landing()
{
  // if(gps.isAvailable)
  // transmit gps info
}

template<class T> void AircraftWrapper<T>::getDatas()
{
  lsm_->readAccel();
  lsm_->readGyro();
  lsm_->readMag();
...
template<class T> void AircraftWrapper<T>::writeDatas()
{
  transmitter_->transmit("("+std::to_string(datas.roll)+", "+std::to_string(datas.pitch)+", "+std::to_string(datas.yaw)+")");
}

template<class T> void AircraftWrapper<T>::onReceive()
{
